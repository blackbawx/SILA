...
class MelVQVAEv4(WaveLSTM12b):
    ...
    def __init__(self, n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025, logits_dim=30,
        ...
        self.joint_encoder = nn.LSTM(321, 256, batch_first=True)
        self.mels2encoderinput = nn.Linear(321,321)
        self.mels2conv = nn.Linear(80,64)
        self.conv_1x1 = nn.Conv1d(64, 64, 1)
        self.conv_1x1.bias.data.zero_()
        self.bn = nn.BatchNorm1d(64, momentum=0.99, eps=1e-3)

    def forward(self, mels, x, emb):
        ...
        emb = emb.unsqueeze(1).expand(B, quantized.shape[1], -1)
        #print("Shapes of embed and quantized: ", emb.shape, quantized.shape)
        quantized = torch.cat([quantized, emb], dim=-1)
        quantized = self.upsample_network(quantized)
        #print("Shape of upsampled quantized latents: ", quantized.shape)

        # Adjust inputs
        quantized = quantized[:,:-1,:]
        inp = x[:, :-1].unsqueeze(-1) * 128 + 128
    ...
    def forward_eval(self, mels, embed, log_scale_min=-50.0):
        ...
        quantized = self.conv_1x1(quantized.transpose(1,2)) #.transpose(1,2)
        quantized = self.bn(quantized).transpose(1,2)
        embed = embed.unsqueeze(0).expand(B, mels.shape[1], -1)
        quantized = torch.cat([quantized, embed], dim=-1)
        mels = self.upsample_network(quantized)

        T = mels.size(1)
        #mels = torch.cat([mels, embed.unsqueeze(0).expand(B, mels.shape[1], -1)], dim=-1)
...
class WaveLSTM(TacotronOne):
    ...
    def __init__(self, n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,
                 r=5, padding_idx=None, use_memory_mask=False, discretizedMoL = 0, spk_embedding_dim=None):
        ...
        self.MoL = discretizedMoL
        if discretizedMoL == 1:
           self.coarse_hidden2logits_coarse = SequenceWise(nn.Linear(64, 30))
        ...
        if spk_embedding_dim is not None:
           self.pre_recurrent_fc = nn.Linear(337, 128)
           self.pre_recurrent_conv1x1_initial = nn.Conv1d(337, 256, 1)
           self.pre_recurrent_conv1x1 = nn.Conv1d(128, 128, 1)
           self.pre_recurrent_conv1x1.bias.data.zero_()
           self.joint_encoder = nn.LSTM(embedding_dim + 1, 128, batch_first=True)
        else:
           self.joint_encoder = nn.LSTM(81, 128, batch_first=True)

        self.hidden2hidden = nn.Linear(128, 64)
        self.hidden2logits = nn.Linear(64, 60)
        ...
        global_cond_channels = embedding_dim
        self.use_kotha = None


    def forward_MoL(self, mels, x):

        B = mels.size(0)
        outputs = {}
        #print("Shape of mels and x: ", mels.shape, x.shape)
        mels = self.upsample_network(mels)
        mels = mels[:,:-1,:]
        inputs = x[:, :-1].unsqueeze(-1)
        #print("Shape of mels and inputs: ", mels.shape, inputs.shape)
        melsNcoarseNfine = torch.cat([mels, inputs], dim=-1)
        
        hidden,_ = self.joint_encoder(melsNcoarseNfine)
        hidden = torch.tanh(self.hidden2hidden(hidden))
        logits = self.hidden2logits(hidden)

        return logits, x[:,1:].unsqueeze(-1)


    def forward_MoL_modified(self, mels, x, emb = None):
        ...
        if emb is None:  
           ...
           c0 = self.conv0(inputs, emb)
           print("Shape of c0: ", c0.shape)
           c1 = self.conv1(c0, emb)
           print("Shape of c1: ", c1.shape)
           c2 = self.conv2(c1, emb)
           print("Shape of c2 and mels: ", c2.shape, mels.shape)
           r0 = self.rnn0(torch.cat(filter_none([c2, mels]), dim=2), emb)[0]
           r1 = self.rnn1(torch.cat([c1[:, (self.delay_r0 - self.delay_c1) // 16:], r0], dim=2), emb)[0]
           r2 = self.rnn2(torch.cat([c0[:, (self.delay_r1 - self.delay_c0) // 4:], r1], dim=2), emb)[0]
           emb = emb.unsqueeze(1).expand(B, mels.shape[1], -1)

        else:
           pass

        melsNinputs = torch.cat([inputs, mels], dim=2)
        hidden,_ = self.joint_encoder(melsNinputs)
        hidden = torch.relu(self.hidden2hidden(hidden))
        logits = self.hidden2logits(hidden)

        return logits, x[:,1:].unsqueeze(-1)


    def forward_eval_MoL(self, mels, log_scale_min=-50.0):
        ...
        input_float = torch.zeros(mels.shape[0], 1).cuda() + 0.0034
        output = []
        hidden = None
        ...
        for i in range(T):
           ...
           hidden_ = torch.relu(self.hidden2hidden(mels_encoded))
           ...
           if i%10000 == 1:
              print("Predicted class at timestep ", i, "is :", sample, " Number of steps: ", T)

           # Generate sample at current time step
           #print("Shapes of categorical_coarse and categorical_fine: ", categorical_coarse.shape, categorical_fine.shape)
           #sample = sample_coarse
           output.append(sample)

           # Estimate the input for next time step
           input_float = torch.FloatTensor([sample])
           input_float = input_float.unsqueeze(0)
...
class QuantVQVAE_MoL(WaveLSTM):  
  
    def __init__(self, n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025, logits_dim=30,  
                 r=5, padding_idx=None, use_memory_mask=False, discretizedMoL = 0, assistant = None):  
        super(QuantVQVAE_MoL, self).__init__(n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,  
                 r=5, padding_idx=None, use_memory_mask=False, discretizedMoL = discretizedMoL)  
  
        self.quantizer = quantizer_kotha(n_channels=1, n_classes=512, vec_len=256, normalize= True, assistant = assistant)  
        encoder_layers = [  
            (2, 4, 1),  
            (2, 4, 1),  
            (2, 4, 1),  
        ...
        self.downsampling_encoder = DownsamplingEncoderStrict(256, encoder_layers)  
        self.decoder = WaveLSTM(n_vocab=257, 
                     embedding_dim=512, 
                     mel_dim=81,
                     discretizedMoL = 1,
                     spk_embedding_dim=128
                     )
        self.decoder.upsample_scales = [2,2,4,4]
        self.decoder.upsample_network = UpsampleNetwork(self.decoder.upsample_scales)
        self.pre_batch_norm = nn.BatchNorm1d(144, momentum=0.9) 
        self.pre_recurrence_conv = nn.Conv1d(144, 144, 1)
        wsize = 2.967 / math.sqrt(336) 
        self.pre_recurrence_conv.weight.data.uniform_(-wsize, wsize) 
        self.pre_recurrence_conv.bias.data.zero_() 
        self.embedding_fc = nn.Linear(256, 256)


    def forward_MoL(self, quants_translated, quants, embedding):

        outputs = {}
        B = quants.size(0)
        #quants_translated_ = quants_translated * (0.02 * torch.randn(quants_translated.size(0), 1).cuda()).exp() + 0.003 * torch.randn_like(quants_translated)
        quants_translated = quants_translated.unsqueeze(-1)
        #quants_translated = self.pre_batch_norm(quants_translated.transpose(1,2)).transpose(1,2)
        mels = self.downsampling_encoder(quants_translated)

        # Get approximate phones
        #quantized, entropy = self.quantizer.get_quantizedindices(mels.unsqueeze(2))
        #print("The latents are ", quantized)
        quantized, vq_penalty, encoder_penalty, entropy = self.quantizer(mels.unsqueeze(2))
        mels = quantized.squeeze(2)
        emb = embedding.unsqueeze(1).expand(B, mels.shape[1], -1)
        emb = torch.tanh(self.embedding_fc(emb))
        mels = torch.cat([mels, emb], dim=-1)
        #mels = self.pre_recurrence_conv(mels.transpose(1,2)).transpose(1,2)

        #mels = self.pre_batch_norm(mels).transpose(1,2)
        logits, outputs = self.decoder.forward_MoL(mels, quants)
        return logits, quants[:,1:].unsqueeze(-1),  vq_penalty.mean(), encoder_penalty.mean(), entropy
    


    def forward_getlatents(self, quants_original, embedding):

        outputs = {}
        mels = self.downsampling_encoder(quants_original.unsqueeze(-1))
    ...
    def forward_eval_MoL(self, quants, embedding):

        outputs = {}
        B = quants.size(0)
        mels = self.downsampling_encoder(quants.unsqueeze(-1))

        # Get approximate phones
        quantized, entropy = self.quantizer.get_quantizedindices(mels.unsqueeze(2))
        print("The latents are ", quantized, entropy)
        #sys.exit()
        ...
        emb = embedding.unsqueeze(1).expand(B, mels.shape[1], -1)
        emb = torch.tanh(self.embedding_fc(emb))
        print("Shape of emb and mels: ", emb.shape, mels.shape)
        mels = torch.cat([mels, emb], dim=-1)

        return self.decoder.forward_eval_MoL(mels), entropy
...
class MelLSTM(nn.Module):
    ...
    def forward(self, mels, x, group_frames = 0):
        ...
        print("Shapes of mels and x: ", mels.shape, x.shape)
        if group_frames:
           print("B and T: ", B, mels.shape[1], mels.shape[1] // self.r)
           mels = mels.contiguous().view(B, mels.size(1) // self.r, -1)
...
class MelVQVAE_L1(nn.Module):   
    def __init__(self, embedding_dim=256, input_dim=80, assistant = None):   
        super(MelVQVAE_L1, self).__init__()
        self.quantizer = quantizer_kotha(n_channels=1, n_classes=512, vec_len=256, normalize= True, assistant = assistant)   
        encoder_layers = [   
            (2, 4, 1),   
            (2, 4, 1),   
            ]   
        self.downsampling_encoder = DownsamplingEncoderStrict(256, encoder_layers, input_dim=80)   
        self.decoder = MelLSTM(input_dim=512)
        self.embedding_fc = nn.Linear(256, 256)
        self.decoder.upsample_scales = [2,2]
        self.decoder.upsample_network = UpsampleNetwork(self.decoder.upsample_scales)

    def forward(self, mels_translated, mels_original, embedding):

        outputs = {}
        B = mels_original.size(0)
        mels = self.downsampling_encoder(mels_translated)
        ...
        # Reconstruction
        logits, outputs = self.decoder(mels, mels_original)

        # Return
        return logits, mels_original[:,1:],  vq_penalty.mean(), encoder_penalty.mean(), entropy

    def forward_getlatents(self, mels_original, embedding):
        outputs = {}
        B = mels_original.size(0)
        #print("Shape of mels: ", mels_translated.shape)
        mels = self.downsampling_encoder(mels_original)
        # Get approximate phones
        quantized, entropy = self.quantizer.get_quantizedindices(mels.unsqueeze(2))
        #print("The latents are ", quantized)
